...
**Current Status:** The library uses TypeScript classes for document definitions and a custom, imperative validation system (`ValidateUtil`) that mutates objects to enforce types. It has recently migrated from `ObjectId` (BSON) to `UUID` (v7).
...
- **`UUID`:** Uses `UUID` (v7) for unique identifiers, replacing the previous dependency on `bson`'s `ObjectId`.
...
- **Boilerplate:** Every field requires a manual function call. Adding a field requires updating the class _and_ the validator.
...
First, install Zod and define a reusable schema for `UUID`.
...
// Custom Zod schema for UUID
export const UuidSchema = z.string().uuid();

export const BaseDocumentSchema = z.object({
  _id: UuidSchema.default(() => uuidv7())
});
```
...
3.  **Frontend Integration:** The same `UserSchema` can be used in the frontend with libraries like `react-hook-form` (via `@hookform/resolvers/zod`) to validate forms _before_ sending data.
...
1.  **Class Methods:** If your current Document classes have methods (logic), you will need to move them to utility functions or use a pattern where the Class wraps the Zod data.
    - _Mitigation:_ Use "Anemic Domain Models" (data only) and separate Service classes for logic, which seems to be the direction of `core-ts-db-lib` anyway.
2.  **Migration Effort:** Requires rewriting all Document files.
    - _Mitigation:_ Can be done incrementally. Keep `BaseDocument` as is while migrating individual subclasses.
...
**Recommended Action:** Start by migrating `User` and `ApiKey` as a pilot to establish the patterns for `UUID` and `BaseDocument` before tackling the more complex `DashboardTask`.
