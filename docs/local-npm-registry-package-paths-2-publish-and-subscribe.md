...
This is the part that makes two publishing directories of one package work at the same time. Versions already
carry the slug of the directory they came from, so what remains is retention per path instead of
deleting the whole package, and a consumer stating which directory it is subscribing to.

The dist tag stays the shared `local`. A tag per path was in this plan, and it turns out to buy
nothing: subscribers are pinned to exact versions and retention filters on the slug inside a version,
so no dist tag is ever read. The tag only exists because npm writes one on every publish and defaults
to `latest`, which is what a constant `local` keeps it off.
...
- New `listPublishedVersions(packageName)`: the versions the registry currently holds for a package,
  empty when it holds none. Narrowing that to one path's versions is what keeps retention per path
  without the store tracking it, and it goes through a new
  `LocalPackageStoreService.filterVersionsForPath`, which keeps the slug private to the service that
  puts it into a version in the first place.
...
- Delete `#clearPublishedPackagesLocally` and `unpublishPackage`, then `src/types/VerdaccioDb.ts`,
  which has no other consumer. Both directories publish under one package name and share one storage
...
- `execute(packageName, packagePath?)`, handing the path to `getPackagePath`, which already requires
  ...
  candidates.
- Which directory the consumer wants cannot be inferred from it, and a wrong guess is the worst
  outcome available, since the consumer then tests against the other directory's build with nothing
  to indicate it. Ancestor matching does not transfer from the cascade's edge resolution, because a
  dependent sits inside one publishing directory while a consumer sits outside all of them, and
  picking by most recently published would rebind on timing.
...
- Before binding, look for an existing subscription for this consumer and package on another path.
  ...
  publishing directories cannot record a timestamped version as the original specifier. A consumer has one
...
- `createTestPackage` takes an optional directory name; two copies of one package name currently
  collide on a directory derived from the name.
- A helper that publishes one test package from two directories.

**`src/commands/PublishCommand.spec.ts`** and **`SubscribeCommand.spec.ts`**: one package published
from two directories with two consumers: each consumer resolves its own build, publishing from one
directory leaves the other's version installed and resolvable, and subscribing with two candidate paths and no
...
**`src/services/Verdaccio.service.spec.ts`**: `unpublishVersions` removing one version while leaving
the other path's version resolvable.

## Step 5: documentation

**`packages/local-npm-registry/README.md`**

- Describe per path retention in the technical details.
...
The suite runs serially (`fileParallelism: false`) and starts a real Verdaccio, so the tests that
publish from two directories should reuse the existing per-test temp instance rather than spawning
