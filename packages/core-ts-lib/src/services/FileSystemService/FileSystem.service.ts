...
import GlobMatchingService from './GlobMatching.service.js';
...
export default class FileSystemService {
  ...
  static async hasPendingChanges(): Promise<boolean> {
    try {
      const { stdout } = await FileSystemService.#execAsync('git status --porcelain');
  ...
  static async hasChangesComparedToMain(absolutePath?: string): Promise<boolean> {
    ...
    try {
      ...
      const { stdout: gitRoot } = await FileSystemService.#execAsync(
      ...
      // Get changed files
      const { stdout } = await FileSystemService.#execAsync(gitCommand, { cwd: repoRoot });
  ...
  static async #getGitDiffCommand(repoRoot: string): Promise<string> {
    try {
      await FileSystemService.#execAsync('git show-ref --verify --quiet refs/remotes/origin/main', {
        cwd: repoRoot
      });
