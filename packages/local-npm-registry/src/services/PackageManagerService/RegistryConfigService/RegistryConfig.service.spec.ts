...
} from './RegistryConfig.service.js';
...
describe('Unit Tests', () => {
  ...
  describe('createRegistryConfig', () => {
    ...
    it('should backup existing configuration files', async () => {
      ...
      expect(newContent).toContain('# Created by local-npm-registry');
      expect(newContent.split('\n')[0]).toBe('# Created by local-npm-registry');
    });
...
@myorg:registry=https://another-custom-registry.com/`;
    ...
    const verifyNpmRegistryConfig = async (
      ...
      expect(npmrcContent).toContain('# Created by local-npm-registry');
      expect(npmrcContent.split('\n')[0]).toBe('# Created by local-npm-registry');
    ...
    const verifyYarn4RegistryConfig = async (
      ...
      expect(yarnrcYmlContent).toContain('# Created by local-npm-registry');
      expect(yarnrcYmlContent.split('\n')[0]).toBe('# Created by local-npm-registry');
  ...
  describe('restoreRegistryConfig', () => {
    it('should restore original configuration files', async () => {
      ...
      expect(currentContent).toContain('# Created by local-npm-registry');
    ...
    it('should recognize and handle generated config files during restoration', async () => {
      const packagePath = await TestProjectUtils.createTestPackage(
        `@test-${testId}/generated-config-test`,
        '1.0.0',
        PackageManager.Npm
      );
      ...
      // Create a file that looks like it was generated by local-npm-registry
      const generatedContent = `# Created by local-npm-registry
registry=http://localhost:4873
@test-${testId}:registry=http://localhost:4873
`;
      await fs.writeFile(npmrcPath, generatedContent);

      // Create backup object as if this was backed up (with null content since it didn't exist originally)
      const backup = {
        npmrc: { path: npmrcPath, content: null }
      };

      // Restore should remove the generated file since it didn't exist originally
      await RegistryConfigService.restoreRegistryConfig(backup);
    ...
    it('should restore original content when generated config overwrote existing file', async () => {
      const packagePath = await TestProjectUtils.createTestPackage(
        `@test-${testId}/generated-overwrite-test`,
        '1.0.0',
        PackageManager.Npm
      );
      ...
      // Create registry config (this will backup the original and overwrite with generated content)
      const backup = await RegistryConfigService.createRegistryConfig(
        PackageManager.Npm,
        DEFAULT_CONFIG.registryUrl,
        packagePath
      ...
      const generatedContent = await fs.readFile(npmrcPath, 'utf8');
      expect(generatedContent).toContain('# Created by local-npm-registry');
      expect(generatedContent).toContain('registry=http://localhost:4873');

      // Restore should replace generated content with original
      await RegistryConfigService.restoreRegistryConfig(backup);
