...
export class RegistryConfigService {
   ...
   * If the current config file starts with our generated comment, it will be completely removed
   * and replaced with the original content.
   *
  ...
  static async restoreRegistryConfig(backup: PackageManagerConfigBackup): Promise<void> {
    for (const config of Object.values(backup)) {
      ...
      } else if (isCurrentConfigGenerated) {
        // If current config was generated by us, remove it completely
        await fs.remove(config.path);
      } else {
        // Fallback to original backup approach if no .tmp file exists and config wasn't generated
  ...
  private static generateRegistryConfig(
    ...
    // Start with the basic registry configuration (no comment header - that's added later)
  ...
  /**
   * Checks if a configuration file was generated by local-npm-registry by looking for the comment header.
   *
